...
<div class="max-lg:mt-4">
    ...
    <div
        ...
        <div class="text-xl font-bold tracking-wider row-span-2 w-full">
            ...
            <div
                class="text-lg font-normal truncate-home-slider-item opacity-0 group-hover:animate-opacity-to-100 mt-2 animate-opacity-to-0 pointer-events-none">
                {!! $post->snippet !!}
            </div>
        </div>
            ...
            <div class="flex gap-1 items-center">
                @if($post->authorImage)
                    <div class="w-8 h-8 rounded-full bg-cover bg-center"
                        style="background-image: linear-gradient(rgba(0,0,0,0.2), rgba(0,0,0,0.2)), url({{ asset($post->authorImage) }})"
                    >
                    </div>
                @else
                    <div>
