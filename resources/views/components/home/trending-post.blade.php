...
<a href="{{ route('post.view', $trendingPost->slug) }}">
    <div>
        ...
        <div
            ...
            <div class="flex text-2xl gap-3 w-full justify-end items-end row-start-3">
                <div class="flex gap-1 items-center">
                    @if($trendingPost->authorImage)
                        <div class="w-8 h-8 rounded-full bg-cover bg-center"
                            style="background-image: linear-gradient(rgba(0,0,0,0.2), rgba(0,0,0,0.2)), url({{ asset($trendingPost->authorImage) }})"
                        >
                        </div>
                    @else
                        <div>
